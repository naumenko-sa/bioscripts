...
for sample in samples:
	...
	with open(family+'.'+sample+'.c4r','w') as f_sample:
	    fieldnames=['Position','Ref','Alt','Variation','Zygosity.'+sample,'Protein_change_ensembl','Gene','Conserved_in_29_mammals','Sift_score','Polyphen_score','Cadd_score']
	    for row in reader:
		l = []
		for key in fieldnames:
		    l.append(row[key])
		zygosity=row['Zygosity.'+sample]
