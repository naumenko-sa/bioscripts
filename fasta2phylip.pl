...
# modified for use with phylip: taxa names should be ten characters length exactly
######################################################################################
use strict;
...
my $line = "";
	...
	if($line =~ /^>(\S+)/) 
	{
		...
		{
		    my $tail;
	    	    for (my $k=1;$k<=10-length($seqName);$k++)
		    {
    			$tail.=" ";
		    }
		    print OUT "$seqName$tail$seq\n";
		    $seq = $seqName = "";
		}
		else 
		{
	...
	else 
	{
		...
		close IN;
			...
			my $len = length $seq;
			    if ($len == $seqLen) {
			    my $len_s = 10 - length($seqName);
			    my $tail;
			    for (my $k=1;$k<=$len_s;$k++)
    		    																																				    {
										    		    																																					        $tail .=" ";
												    		    																																					    	    }
														    		    																																					    		    print OUT "$seqName$tail$seq\n";
