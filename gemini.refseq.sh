...
# 1.remove VEP ensembl annotation
vt rminfo ${family}-ensemble.vcf.gz -t CSQ -o ${family}.no_vep.vcf.gz

# 2.decompose vcf, because reference ENSEMBL variants are decomposed
gemini.decompose.sh ${family}.no_vep.vcf.gz

# 3.annotate with refseq
gemini.vep.refseq.sh ${family}.no_vep.decomposed.vcf.gz

# 4.extract positions of the variants in the final report
cat ${family}-ensemble.db.txt  | awk -F "\t" '{print $23"\t"$24}' | sed 1d | sed s/chr// > $family.report.tab

# 5.extract variants with their VEP annotations
bcftools view -o $family.tosort.vcf -R $family.report.tab ${family}.no_vep.decomposed.vepeffects_refseq.vcf.gz 

# 6.sort vcf
picard SortVcf I=$family.tosort.vcf O=$family.refseq.vcf

# 7.convert vcf to table
